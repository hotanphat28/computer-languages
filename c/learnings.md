...
2. [Fundamentals of Programming](#fundamentals-of-programming)
3. [Terminal | Command-line](#terminal--command-line)
4. [Algorithm Theory](#algorithm-theory)
5. [Memory Theory](#memory-theory)
6. [C Programming](#c-programming)
    ...
    - [File I/O](#file-io-file-pointers)
...
* **Abstract**: The art of hiding complexity. You use a feature (like a car engine or a sort function) without needing to know *how* it works internally, just *how to use it*.
* **Algorithm**: A step-by-step set of instructions to solve a specific problem. Like a recipe.
* **Pseudocode**: A human-readable outline of your logic. It borrows syntax from programming but is meant for humans to plan out code before writing it.
* **Functions**: Modular blocks of code that perform a specific task.
  * **Arguments**: Inputs you pass *into* the function.
  * **Return values**: The output the function gives *back*.
  * **Side effects**: Things the function does besides returning a value (e.g., printing to the console, changing a global variable).
* **Conditionals**: Decision-making points (e.g., "If X is true, do Y").
* **Boolean expressions**: Questions that evaluate to either `true` or `false` (e.g., `5 < 10` is True).
* **Loops**: Repeating a block of code multiple times (e.g., "Print 'Hello' 50 times").
* **Variables**: Named containers for storing data values (e.g., `score = 100`).
...
### File Manipulation
* `cp <source> <dest>` - **Copy** a file. (`cp -r` for directories).
* `mv <source> <dest>` - **Move** (or rename) a file.
* `rm <file>` - **Remove** (delete) a file. 
  * `rm -rf <dir>` - Force remove a directory and its contents (Careful!).
* `mkdir <dir>` - **Make Directory**.
* `rmdir <dir>` - **Remove Directory** (only if empty).
* `touch <file>` - Create an empty file or update timestamp.

### Permissions & System
* `chmod` - **Change Mode** (permissions). e.g., `chmod +x script.sh` to make it executable.
* `sudo` - **SuperUser DO**. Execute command with admin privileges.
* `man <command>` - **Manual**. Read the documentation for a command.

### Misc
* `cat <file>` - Read file content to screen.
* `diff <file1> <file2>` - Compare two files.
* `echo "text"` - Print text to output.
* `telnet` - Communicate with another host (older protocol, distinct from SSH).

---

## Algorithm Theory

**Running Time** (Performance): How many steps does it take to solve the problem as the input size ($n$) grows? We measure this in terms of *time* (speed) and *space* (memory).

### Big O ($O$) - Upper Bound (Worst Case)
Describes the *maximum* number of steps an algorithm will take. We want to know: "If everything goes wrong, how slow will this be?"

*   **$O(n^2)$**: **Selection Sort, Bubble Sort**. Slow. For each element, you might have to check every other element (loop within a loop).
*   **$O(n \log n)$**: **Merge Sort**. Fast. You split the problem in half repeatedly ($\log n$) and process elements ($n$).
*   **$O(n)$**: **Linear Search**. Okay. You assume the answer is the last thing you check.
*   **$O(\log n)$**: **Binary Search**. Very fast. You cut the search area in half with every step.
*   **$O(1)$**: **Constant Time**. Instant. The size of the input doesn't matter (e.g., accessing an array index).

### Omega ($\Omega$) - Lower Bound (Best Case)
Describes the *minimum* number of steps. "If we get lucky, how fast can this be?"

*   **$\Omega(n^2)$**: Selection Sort (always checks everything).
*   **$\Omega(n \log n)$**: Merge Sort (always splits and merges).
*   **$\Omega(n)$**: Bubble Sort (if the list is already sorted, it just checks once).
*   **$\Omega(1)$**: Linear Search (if the first item is the one you want).

### Theta ($\Theta$) - Tight Bound
When the Best Case and Worst Case are the same class of complexity.
...
#### Compilation Process
Code doesn't run by itself. It goes through four stages:
1.  **Preprocessing**: Handles lines starting with `#` (like `#include`).
2.  **Compiling**: Converts C code into Assembly code.
3.  **Assembling**: Converts Assembly into Machine Code (0s and 1s).
4.  **Linking**: Combines your code with libraries (like `cs50.c`) into one executable file.

**Commands:**
*   `make <name>`: Automated way to compile.
*   `clang -o hello hello.c`: Manual compilation.
*   `./hello`: Run the executable.

#### argc and argv
The `main` function can accept arguments from the terminal!

```c
int main(int argc, string argv[])
```

*   **argc (Argument Count)**: An integer representing how many arguments were typed.
    *   Note: The program name itself counts as 1. So `argc` is always at least 1.
*   **argv (Argument Vector)**: An array of strings containing the arguments.
    *   `argv[0]`: The program name (e.g., `./hello`).
    *   `argv[1]`: The first word you typed after the program name.
...
{
	string s = "HI!";
	char *s_ptr = &s; // Renamed to avoid conflict with string s
	char *s_literal = "HI!"; // Renamed to avoid conflict with string s
	printf("%s\n", s); // it will print HI!
...
Choosing the right data structure is a trade-off between speed (search/sort) and flexibility (insertion/deletion).

*   **Arrays**: Best for random access (I want item #5). Bad for inserting/deleting (have to move everything). Fixed size.
*   **Linked Lists**: Best for inserting/deleting (just change pointers). Bad for searching (have to walk the chain). Dynamic size.
*   **Hash Tables**: The "magic" structure. Almost instant insertion, deletion, and search (O(1)). Uses more memory and cannot be easily sorted.
*   **Tries**: Great for dictionaries/strings. Fast lookup but can use huge amounts of memory.

| Structure | Insertion | Deletion | Search | Sorted? | Notes |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Arrays** | Slow (Shift items) | Slow (Shift items) | Great (O(1) if index known, O(log n) if sorted) | Easy | Fixed size. |
| **Linked Lists** | Fast (O(1)) | Fast (O(1)) | Slow (O(n)) | Hard | Dynamic size. |
| **Hash Tables** | Fast (O(1)) | Fast (O(1)) | Fast (O(1)) | No | Memory heavy. |
| **Tries** | Complex | Fast | Fast | Yes | Huge memory usage. |
...
![Singly linked lists](./assets/singly-linked-lists.png "Singly linked lists")
...
![Double linked lists 1](./assets/double-linked-lists-1.png "Double linked lists 1")

![Double linked lists 2](./assets/double-linked-lists-2.png "Double linked lists 2")
...
![Hash tables 1](./assets/hash-tables-1.png "Hash tables 1")

has function + array

![Hash tables 2](./assets/hash-tables-2.png "Hash tables 2")

how linear probing works to solve clustering

chaining to solve collisions

![Hash tables 3](./assets/hash-tables-3.png "Hash tables 3")

![Hash tables 4](./assets/hash-tables-4.png "Hash tables 4")
